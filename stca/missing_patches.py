"""Missing-patch detection — inspired by Google Vanir.

Vanir compares your source code against known security patch diffs to find
code that hasn't been patched for known CVEs. This is different from SCA
(which checks package versions) — it checks the ACTUAL CODE for whether
a specific fix has been applied.

How it works:
  1. Maintain a database of known security patches (CVE → patch diff)
  2. For each CVE, extract the "vulnerable pattern" (the code BEFORE the fix)
  3. Scan your code for that pattern
  4. If found → you're running unpatched code for that CVE

This catches:
  - Packages where you've backported a fix but missed a file
  - Vendored/forked code that hasn't been updated
  - Code where the version says "fixed" but the patch wasn't actually applied

Vanir was built for Android but the concept applies to any codebase.
We implement a simplified version: a curated database of high-profile CVE
patches with their vulnerable code patterns.
"""
...
PATCH_DATABASE: List[SecurityPatch] = [
    SecurityPatch(
        cve="CVE-2021-23337",
        package="lodash",
...
def scan_missing_patches(repo_root: Path,
...
def patch_database_stats() -> dict:
    """Return stats about the patch database."""
    from collections import Counter
    by_severity = Counter(p.severity for p in PATCH_DATABASE)
    by_language = Counter(p.language for p in PATCH_DATABASE)
    return {
        "total_patches": len(PATCH_DATABASE),
