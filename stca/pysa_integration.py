...
This module:
  ...
  - Parses Pysa's JSON output into STCA Findings
  - Falls back to STCA's built-in CPG taint tracker if Pysa isn't available

To install Pysa:
  pip install pyre-check
  # then run `pyre init` in your repo

Pysa is much more accurate than STCA's CPG taint tracker because:
...
class PysaIntegration:
    ...
    def install_instructions(self) -> str:
        return (
            ...
            "Then re-run `stca check` — Pysa findings will be included automatically.\n"
            "Pysa is Meta's open-source taint analyzer, tuned across millions of lines of Python.\n"
            "It catches what STCA's built-in CPG taint tracker misses."
        )
...
def get_pysa_findings_or_fallback(repo_root: Path,
    ...
    upgrades STCA's taint analysis when Pysa is installed.
    """
