...
`.stca-invariants.json`). On each run:

  1. For functions in the diff, check that the previously inferred invariants
     still hold (cheap, deterministic, <1s).
  2. If `.stca-invariants.json` is missing or stale, the user can run
     `stca bootstrap invariants` to re-infer from a test run.
...
INVARIANTS_FILE = ".stca-invariants.json"
    ...
    def run(self, repo_root: Path, hunks: List[DiffHunk],
        ...
        if not inv_file.exists():
            findings.append(Finding(
                ...
                message="No invariant database found — run `stca bootstrap invariants` to infer invariants from a test run",
