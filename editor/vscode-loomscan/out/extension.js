...
function activate(context) {
    ...
    stcaClient.start();
    // Commands
    context.subscriptions.push(vscode.commands.registerCommand('loomscan.checkRepo', () => stcaClient?.runRepoCheck()), vscode.commands.registerCommand('loomscan.checkFile', () => stcaClient?.runFileCheck()), vscode.commands.registerCommand('loomscan.applyFix', () => stcaClient?.applyFix()), vscode.commands.registerCommand('loomscan.showUncertain', () => stcaClient?.showUncertain()), vscode.commands.registerCommand('loomscan.gate', () => stcaClient?.runGate()), vscode.commands.registerCommand('loomscan.restart', async () => {
...
class StcaLspClient {
    ...
    async startLspServer() {
        ...
        this.languageClient = new node_1.LanguageClient('loomscan', 'LoomScan', serverOptions, clientOptions);
    ...
    renderDiagnostics(doc, findings) {
        ...
        const diags = fileFindings.map(f => {
            ...
            const diag = new vscode.Diagnostic(range, `[LoomScan ${f.rule_id}] ${f.message}`, severity);
    ...
    async applyFix() {
        ...
        const choice = await vscode.window.showInformationMessage(`LoomScan: applying fix for ${diag.code}...`, 'Apply', 'Cancel');
        if (choice !== 'Apply')
            return;
        const config = vscode.workspace.getConfiguration('loomscan');
        const pythonPath = config.get('pythonPath', 'python');
    ...
    async showUncertain() {
        ...
        vscode.window.showInformationMessage(`LoomScan: ${newState ? 'showing only uncertain (30-70%) findings' : 'showing all findings'}`);
        if (this.fallbackMode) {
            vscode.workspace.textDocuments.forEach(doc => this.analyzeFile(doc));
        }
    }
    async runGate() {
        ...
        const args = ['-c', 'from loomscan.cli import main; main()', 'gate', '--full'];
        if (preset === 'strict') {
